            ...
            <tbody>
                <?php if($Managers){ foreach($Managers as $Manager){ $Role = $Manager->user->role; ?>
                    <tr class="tr_list1">
                        ...
                        <td class="border-b-2px-solid-color3 p-0 text-center"><a class="text-color2 no-hover" href="<?=Yii::$app->request->baseUrl.'/'.'manager/'.$Manager->IdManager?>"><div class="w-100 h-100 p-3"><?php if($Manager->user->Gender == 'M'){ echo 'Man';}else{if($Manager->user->Gender == 'F'){echo 'Woman';}else{echo 'Unknow';}}?></div></a></td>
