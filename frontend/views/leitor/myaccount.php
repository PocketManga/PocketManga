...
/* @var $model backend\models\Leitor */
    ...
    <div class="container-fluid pb-4 px-4">
        <div class="row">
            ...
            <div class="col-2 py-2">
                <a href="" id="update-button" data-toggle="modal" data-target="#updateRole" class="btn btn-primary w-100 background-color3"><span class="text">Update</span></a>
            </div>
            <div class="col-2 py-2">
            ...
            <div class="col-12 align-center my-4 pt-4 border-t-2px-solid-color3">
                ...
                <img class="radi-all-50p" src="<?=Yii::$app->urlManagerBackend->baseUrl.'/img'.Yii::$app->user->identity->SrcPhoto?>" height="300" width="300">
                <?php }else{ ?>
                <img class="radi-all-50p" src="<?=Yii::$app->urlManagerBackend->baseUrl.'/img/default/'.((Yii::$app->user->identity->Gender == "F")?'F':'M').'.jpg'?>" height="300" width="300">
                <?php }}else{ ?>
                <img class="radi-all-50p" src="<?=Yii::$app->urlManagerBackend->baseUrl.'/img/default/'.((Yii::$app->user->identity->Gender == "F")?'F':'M').'.jpg'?>" height="300" width="300">
                <?php } ?>
            </div>
                    ...
                    <div class="col-12 mb-4">
                        <div class="row bold">
                            <div class="col-6 mb-4">
                                <p class="text-color6">Username: <span class="text-color1"><?=Yii::$app->user->identity->Username?></span></p>
                            </div>
                            <div class="col-6 mb-4">
                                <p class="text-color6">Email: <span class="text-color1"><?=Yii::$app->user->identity->Email?></span></p>
                            </div>
                            <div class="col-6 mb-4">
                                <p class="text-color6">Gender: <span class="text-color1"><?php if(Yii::$app->user->identity->Gender == 'M'){ echo 'Man';}else{if(Yii::$app->user->identity->Gender == 'F'){echo 'Woman';}else{echo 'Unknow';}}?></span></p>
                            </div>
                            <div class="col-6 mb-4">
                                <p class="text-color6">Birth Date: <span class="text-color1"><?=(new DateTime(Yii::$app->user->identity->BirthDate))->format('d/m/Y')?></span></p>
                            </div>
                            <div class="col-6 mb-4">
                                <p class="text-color6">Server: <span class="text-color1"><?php foreach($Servers as $Server){if(Yii::$app->user->identity->leitor->Server == $Server->Code){echo $Server->Name;}}?></span></p>
                            </div>
                            <div class="col-6 mb-4">
                                <p class="text-color6">Theme: <span class="text-color1"><?=(Yii::$app->user->identity->leitor->Theme)?'Dark':'Light'?></span></p>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="form-row mt-2 mb-3">
                        <div class="col-12">
                            <div class="row">
                                <div class="col-12 align-center">
                                    <?php if(Yii::$app->user->identity->SrcPhoto){ if (file_exists(Yii::getAlias('@backend').'/web/img'.Yii::$app->user->identity->SrcPhoto)){ ?>
                                        <a href="#" onclick="return ClickChange()">
                                            <img class="radi-all-50p" id="uploadPreview" width="250px" height="250px" src="<?=Yii::$app->urlManagerBackend->baseUrl.'/img'.Yii::$app->user->identity->SrcPhoto?>"/>
                                        </a>
                                        <?= $form->field($model, 'Photo')->fileInput(['id' => 'uploadImage', 'onchange'=>'PreviewImage("'.Yii::$app->urlManagerBackend->baseUrl.'/img'.Yii::$app->user->identity->SrcPhoto.'");', 'style'=>'display:none;'])->label(false);?>
                                    <?php }else{ ?>
                                        <a href="#" onclick="return ClickChange()">
                                            <img class="radi-all-50p" id="uploadPreview" width="250px" height="250px" src="<?=Yii::$app->urlManagerBackend->baseUrl.'/img/default/manga_alternative.jpg'?>"/>
                                        </a>
                                        <?= $form->field($model, 'Photo')->fileInput(['id' => 'uploadImage', 'onchange'=>'PreviewImage("'.Yii::$app->urlManagerBackend->baseUrl.'/img/default/manga_alternative.jpg'.'");', 'style'=>'display:none;'])->label(false);?>
                                    <?php }}else{ ?>
                                        <a href="#" onclick="return ClickChange()">
                                            <img class="radi-all-50p" id="uploadPreview" width="250px" height="250px" src="<?=Yii::$app->urlManagerBackend->baseUrl.'/img/default/addImg2.png'?>"/>
                                        </a>
                                        <?= $form->field($model, 'Photo')->fileInput(['id' => 'uploadImage', 'onchange'=>'PreviewImage("'.Yii::$app->urlManagerBackend->baseUrl.'/img/default/addImg2.png'.'");', 'style'=>'display:none;'])->label(false);?>
                                    <?php } ?>
                                </div>
                                <div class="col-12">
                                ...
                                <div class="col-12">
                                    <label class="text-gray-900">Server <sup class="text-danger small">&#10033;</sup></label>
                                    <div class="form-group field-myaccount-theme">
                                        <select id="myaccount-theme" class="radi-all-15 p-1 background-color1 border-0 w-100 text-color2 align-center" name="MyAccountForm[Server]">
                                            <?php foreach($Servers as $Server){ ?>
                                            <option value="<?=$Server->Code?>" <?=(Yii::$app->user->identity->leitor->Server == $Server->Code)?'selected':''?>><?=$Server->Name?></option>
                                            <?php } ?>
                                        </select>
                                    </div>
                                </div>
                                <div class="col-12">
                                    ...
                                    <div class="form-group field-myaccount-theme">
                                        <select id="myaccount-theme" class="radi-all-15 p-1 background-color1 border-0 w-100 text-color2 align-center" name="MyAccountForm[Theme]">
                                            <option value="0" <?=(Yii::$app->user->identity->leitor->Theme == 0)?'selected':''?>>Light</option>
                                            <option value="1" <?=(Yii::$app->user->identity->leitor->Theme == 1)?'selected':''?>>Dark</option>
                                        </select>
                                    </div>
                                </div>
                            </div>
